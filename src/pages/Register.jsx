...
const Register = () => {
  ...
  return (
    <div className="bg-[var(--bg)] text-[var(--text)] min-h-screen flex items-center justify-center px-6 py-24 relative overflow-hidden">
      ...
      >
        <p className="uppercase tracking-[0.3em] text-[var(--muted)] text-xs mb-2 text-center">Get Started</p>
        <h1 className="text-2xl font-bold mb-1 text-center gradient-text-secondary">Create Account</h1>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative">
            <FiUser className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--muted)]" size={16} />
            <input type="text" name="name" value={form.name} onChange={handleChange} placeholder="Full Name" required
              className="w-full bg-[var(--card-bg)] border border-[var(--border)] rounded-xl pl-11 pr-4 py-3 outline-none focus:border-[#A1FFCE]/50 transition-colors placeholder:text-[var(--muted)] text-[var(--text)] text-sm" />
          </div>
          <div className="relative">
          ...
          <div className="relative">
            ...
            <input type={showPassword ? "text" : "password"} name="password" value={form.password} onChange={handleChange} placeholder="Password" required
              className="w-full bg-[var(--card-bg)] border border-[var(--border)] rounded-xl pl-11 pr-11 py-3 outline-none focus:border-[#A1FFCE]/50 transition-colors placeholder:text-[var(--muted)] text-[var(--text)] text-sm" />
