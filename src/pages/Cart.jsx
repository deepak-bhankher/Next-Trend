...
const Cart = () => {
  ...
  if (cartItems.length === 0) {
    return (
      <div className="bg-[var(--bg)] text-[var(--text)] min-h-screen pt-32 pb-20 flex flex-col items-center justify-center text-center px-6">
        <FiShoppingBag size={48} className="text-[var(--muted)] mb-4" />
        <h2 className="text-2xl font-bold mb-2">Your cart is empty</h2>
        <p className="text-[var(--muted)] mb-8">Looks like you haven't added any shoes yet.</p>
    ...
    <div className="bg-[var(--bg)] text-[var(--text)] min-h-screen pt-32 pb-20">
      <div className="max-w-5xl mx-auto px-6">
            ...
            {cartItems.map((item) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-[var(--text)] text-sm sm:text-base truncate">{item.name}</h3>
                  <p className="text-[var(--muted)] text-xs sm:text-sm">Size: {item.size}</p>
                ...
                <div className="flex items-center gap-2 glass-btn rounded-full px-2.5 py-1.5 flex-shrink-0">
                  <button onClick={() => updateQty(item.product, item.size, item.qty - 1)} className="text-[var(--muted)] hover:text-[#B6FBFF] transition-colors">
                    <FiMinus size={13} />
                  </button>
                  <span className="w-4 text-center text-sm text-[var(--text)]">{item.qty}</span>
                  <button onClick={() => updateQty(item.product, item.size, item.qty + 1)} className="text-[var(--muted)] hover:text-[#A1FFCE] transition-colors">
                    <FiPlus size={13} />
                  </button>
                </div>
                <button onClick={() => removeFromCart(item.product, item.size)} className="text-[var(--muted)] hover:text-red-400 transition-colors flex-shrink-0">
                  <FiTrash2 size={16} />
                </button>
              </motion.div>
            ))}
        ...
        <div className="glass-card rounded-2xl p-4 sm:p-6 flex justify-between items-center shadow-xl shadow-black/40">
          <div>
            <p className="text-[var(--muted)] text-sm">Subtotal</p>
